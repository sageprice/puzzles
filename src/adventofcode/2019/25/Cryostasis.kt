...
fun main() {
    ...
    val programSpec: List<Long> =
        File("src/adventofcode/2019/25/input.txt")
            .readLines()[0].split(",").map { it.toLong() }

    // Part 1
    var exec = evalSpec(programSpec, listOf())
//    val items = listOf(
//        "asterisk",
////        "pointer",
//        "mutex",
...
////        "monolith",
////        "mouse",
////        "sand", // Note: sand is too heavy by itself
//        "food ration")

    // Solution was: mutex, asterisk, space law space brochure, food ration
    while (exec.exitInstr != HaltOp) {
        val o = exec.outputs.map { it.toInt().toChar() }
        println(o.joinToString(""))
        val input = if (o.isEmpty()) listOf('\n'.toLong()) else readLine()!!.map { it.toLong() }
        exec = evalSpec(exec.prog, input, exec.startIndex, exec.relativeOffset, true)
    }
    println(exec.outputs.map { it.toChar() }.joinToString(""))
