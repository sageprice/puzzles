package adventofcode.`2019`.`19`

import java.io.File

/**
 * Solution for [Advent of Code day 19](https://adventofcode.com/2019/day/19).
 */
fun main() {
    // Read input file
    val programSpec: List<Long> =
        File("src/adventofcode/2019/19/input.txt")
            // File("test_input.txt")
    ...
    (0L..49).forEach { x ->
        (0L..49).forEach { y ->
            if (evalSpec(programSpec, listOf(x, y)).outputs.last() == 1L) {
                pulledSpots++
            }
        }
    }
    println("Part 1: $pulledSpots")

    // Part 2
    var lStart = 0L
    for (y in generateSequence(100L, { it + 1})) {
        while (evalSpec(programSpec, listOf(lStart, y)).outputs.last() == 0L) {
            lStart++
        }

        if (evalSpec(programSpec, listOf(lStart + 99, y - 99)).outputs.last() == 1L) {
            println("Part 2: ${lStart * 10_000 + y - 99}")
            break
        }
    }
}
